#!/usr/bin/env python3
"""60MB大文件上传测试脚本

用于验证流式上传功能是否正常工作。

使用方法:
    cd upload_tools
    source ~/anaconda3/etc/profile.d/conda.sh && conda activate ShareUSTC_upload
    python tests/test_60mb_upload.py --config config.yaml

参数:
    --config: 配置文件路径
    --cleanup: 测试完成后删除测试文件
"""

import argparse
import os
import sys
import tempfile
import time
from pathlib import Path

# 添加 src 到路径
sys.path.insert(0, str(Path(__file__).parent.parent / "src"))

from config import load_config, resolve_path
from auth import AuthManager
from uploader import BatchUploader
from csv_parser import UploadTask
from utils import format_file_size, print_info, print_success, print_error


def create_test_file(size_mb: int = 60) -> Path:
    """创建指定大小的测试文件
    
    Args:
        size_mb: 文件大小（MB），默认60MB
        
    Returns:
        测试文件路径
    """
    # 创建临时文件
    fd, filepath = tempfile.mkstemp(suffix=".bin", prefix="test_upload_60mb_")
    os.close(fd)
    
    filepath = Path(filepath)
    chunk_size = 1024 * 1024  # 1MB
    total_bytes = size_mb * chunk_size
    
    print_info(f"创建测试文件: {filepath}")
    print_info(f"目标大小: {format_file_size(total_bytes)}")
    
    # 写入随机数据
    with open(filepath, 'wb') as f:
        written = 0
        while written < total_bytes:
            to_write = min(chunk_size, total_bytes - written)
            data = os.urandom(to_write)
            f.write(data)
            written += to_write
            
            # 显示进度（每10MB）
            if written % (10 * chunk_size) == 0:
                progress = (written / total_bytes) * 100
                print(f"  写入进度: {progress:.0f}% ({format_file_size(written)})")
    
    actual_size = filepath.stat().st_size
    print_success(f"测试文件创建完成: {format_file_size(actual_size)}")
    
    return filepath


def test_upload(filepath: Path, config_path: str):
    """测试上传功能
    
    Args:
        filepath: 测试文件路径
        config_path: 配置文件路径
    """
    print_info("加载配置...")
    config = load_config(config_path)
    
    # 创建 AuthManager
    print_info("初始化认证...")
    cookie_file = resolve_path(config.paths.cookie_file)
    
    auth = AuthManager(
        base_url=config.server.base_url,
        cookie_file=cookie_file,
    )
    
    # 检查登录状态
    if not auth.is_authenticated():
        print_error("未登录，请先运行: python -m src.cli --login")
        sys.exit(1)
    
    print_success("已登录")
    
    # 创建上传器 - 使用较长的超时时间（60MB / 1MB/s = 60s，留余量到 180s）
    uploader = BatchUploader(
        base_url=config.server.base_url,
        session=auth.session,
        timeout=300,  # 5分钟，大文件会动态计算更长的时间
    )
    
    # 创建上传任务
    task = UploadTask(
        file_path=filepath,
        title=f"测试大文件上传_60MB",
        course_name="测试课程",
        resource_type="课件",
        category="test",
        tags=["test", "large_file", "60mb"],
        description=f"60MB大文件上传测试，验证流式上传功能",
        row_number=1,
    )
    
    print()
    print("=" * 60)
    print_info("开始上传测试...")
    print(f"  文件: {filepath.name}")
    print(f"  大小: {format_file_size(filepath.stat().st_size)}")
    print(f"  预估上传时间: 约 {filepath.stat().st_size / (1024*1024)} 秒 (按 1MB/s 计算)")
    print("=" * 60)
    print()
    
    start_time = time.time()
    result = uploader.upload_single(task)
    duration = time.time() - start_time
    
    print()
    print("=" * 60)
    if result.success:
        print_success(f"✓ 上传成功！")
        print(f"  资源ID: {result.resource_id}")
        print(f"  耗时: {duration:.2f}秒")
        
        # 计算实际上传速度
        speed_mbps = filepath.stat().st_size / duration / (1024 * 1024)
        print(f"  平均速度: {speed_mbps:.2f} MB/s")
        print("=" * 60)
        return True
    else:
        print_error(f"✗ 上传失败")
        print(f"  错误: {result.error}")
        print("=" * 60)
        return False


def main():
    parser = argparse.ArgumentParser(description="60MB大文件上传测试")
    parser.add_argument("--config", "-c", default="config.yaml",
                        help="配置文件路径 (默认: config.yaml)")
    parser.add_argument("--cleanup", action="store_true",
                        help="测试完成后删除测试文件")
    
    args = parser.parse_args()
    
    print("=" * 60)
    print("60MB大文件上传测试")
    print("=" * 60)
    print()
    
    # 创建60MB测试文件
    test_file = create_test_file(60)
    
    try:
        # 执行测试
        success = test_upload(test_file, args.config)
        
        if success:
            print()
            print("=" * 60)
            print_success("测试通过！大文件上传功能正常")
            print("=" * 60)
        else:
            print()
            print("=" * 60)
            print_error("测试失败！请检查错误信息")
            print("=" * 60)
            sys.exit(1)
            
    finally:
        # 清理测试文件
        if args.cleanup and test_file.exists():
            print()
            print_info(f"删除测试文件: {test_file}")
            test_file.unlink()


if __name__ == "__main__":
    main()
