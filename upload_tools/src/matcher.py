...
class EntityMatcher:
    ...
    def _load_courses(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.debug(f"课程列表: {[str(e) for e in self._courses[:5]]}...")
                return True
            else:
    ...
    def _load_teachers(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.debug(f"教师列表: {[str(e) for e in self._teachers[:5]]}...")
                return True
            else:
    ...
    def _interactive_select(self, query: str, candidates: List[Tuple[float, Entity]], entity_type: str) -> Optional[int]:
        """交互式选择实体
        ...
        while True:
            try:
                ...
                if choice_num == 0:
                    ...
                    selected = candidates[choice_num - 1][1]
                    print(f"  ✓ 已选择: {selected.name} (SN: {selected.sn})")
                    return selected.sn
                else:
                    print(f"  请输入 0 到 {len(candidates)} 之间的数字")
    ...
    def match_course(self, course_name: str, interactive: bool = True) -> Optional[int]:
        """匹配课程
        ...
        if interactive:
            ...
            if top_matches and top_matches[0][0] > self.SIMILARITY_THRESHOLD:  # 最高相似度大于阈值才显示
                return self._interactive_select(course_name, top_matches, "课程")

        logger.warning(f"未找到匹配的课程: {course_name}")
        return None

    def match_teacher(self, teacher_name: str, interactive: bool = True) -> Optional[int]:
        """匹配教师
        ...
        if interactive:
            ...
            if top_matches and top_matches[0][0] > self.SIMILARITY_THRESHOLD:  # 最高相似度大于阈值才显示
                return self._interactive_select(teacher_name, top_matches, "教师")

        logger.warning(f"未找到匹配的教师: {teacher_name}")
        return None
    
    def match_courses(self, course_names: List[str], interactive: bool = True) -> Tuple[List[int], List[str]]:
        """批量匹配课程
        ...
        for name in course_names:
            sn = self.match_course(name, interactive=interactive)
            if sn:
                matched.append(sn)
            else:
    ...
    def match_teachers(self, teacher_names: List[str], interactive: bool = True) -> Tuple[List[int], List[str]]:
        """批量匹配教师
        ...
        for name in teacher_names:
            sn = self.match_teacher(name, interactive=interactive)
            if sn:
                matched.append(sn)
            else:
                unmatched.append(name)
...
def match_task_entities(task: "UploadTask", matcher: EntityMatcher, interactive: bool = True) -> Tuple[List[str], List[str]]:
    """匹配任务中的实体
    ...
    if task.related_courses:
        ...
        matched_sns, unmatched = matcher.match_courses(task.related_courses, interactive=interactive)
    ...
    if task.related_teachers:
        ...
        matched_sns, unmatched = matcher.match_teachers(task.related_teachers, interactive=interactive)
